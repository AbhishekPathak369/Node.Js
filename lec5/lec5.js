...
const PORT = 3000;
server.listen(PORT, () => {
  console.log(`Server running at http://localhost:${PORT}`); // Confirm server is up
});

//  now we create two js separate file where in first we do server work and in second  we do task create  temp.js ans ser(server.js);